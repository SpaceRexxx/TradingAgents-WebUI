...
def create_market_analyst(llm):
    ...
    def market_analyst_node(state):
        ...
        prompt_content = f"请开始进行深入的市场与技术面分析。当前日期是 {current_date}，我们当前要分析的公司是 {ticker}。重点：请在调用获取指标工具时，将 look_back_days 参数设为 {lookback_days}。注意：不需要向我交代工具调用的话语，直接输出排版精美的最终报告和表格即可。"
